...
enum LogType {
    /// Use 'info' informative tasks. There are not considered analytics, issues or errors
    case info
    /// Default type for analytics
    case analytic
    /// Issues or errors that should not occur, but will not negatively affect the user experience
    case warning
    /// Issues or error that negatively affect user experience
    case severe
    ...
    var OSLogType: OSLogType { // Big letter because it is a specific framework ;)
        switch self {
        case .info:
            return .info
        case .analytic:
